...
class StudentReportViewModel(
    ...
    private fun loadData() {
        viewModelScope.launch {
            ...
            try {
                ...
                // Attendance detail
                val attendanceList = attendanceDao.getAttendanceByStudentAndDateRange(studentId, startMillis, endMillis).first()
                val presentCount = attendanceList.count { it.isPresent }
                val totalPassed = attendanceDao.getTotalPassedInRange(studentId, startMillis, endMillis)
                val totalRetake = attendanceDao.getTotalRetakeInRange(studentId, startMillis, endMillis)

                // Perkembangan bacaan
                val sortedAttendance = attendanceList.filter { it.isPresent }.sortedBy { it.date }
                val awal = sortedAttendance.firstOrNull()
                val akhir = sortedAttendance.lastOrNull()
    ...
    private fun getReadingPosition(att: com.example.myapplication.data.Attendance): String {
        return when {
            att.iqroNumber != null && att.iqroPage != null -> "Iqro ${att.iqroNumber} Hal ${att.iqroPage}"
            att.quranSurah != null && att.quranAyat != null -> "Surat ${getSurahName(att.quranSurah)} ayat ${att.quranAyat}"
            else -> "-"
        }
    }
